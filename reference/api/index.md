...
- **`escape`** – Escape text for ansiprint by replacing < and > with special strings.
...
- **`unescape`** – Unescape text by replacing special strings with < and >.
...
- **`args`** (`Sequence`) – The \*args to pass when calling.
- **`kwargs`** (`Mapping`) – The \*\*kwargs to pass when calling.
...
- **`args`** (`Sequence`) – The \*args to pass when calling.
- **`call`** (`Callable[_P, _R]`) – The original callable.
- **`kwargs`** (`Mapping`) – The \*\*kwargs to pass when calling.
- **`name`** (`str | None`) – The name of the callable, if any.
